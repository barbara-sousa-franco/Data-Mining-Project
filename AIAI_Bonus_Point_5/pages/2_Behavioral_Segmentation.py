# For 2_Behavioral_Segmentation.py
...
def load_data():
    ...
    df = df_unscaled.copy()
    df['cluster'] = df_scaled['behav_labels']
    
    return df

st.title("Behavioral Segmentation (All Customers – K-Means, 5 clusters)")
...
# 3 most variant features for default 3D axes
variance = filtered_df[numeric_cols].var().sort_values(ascending=False)
top3 = variance.index[:3].tolist()

col1, col2, col3 = st.columns(3)
with col1:
    x = st.selectbox("X-axis", numeric_cols, index=numeric_cols.index(top3[0]), key="bx")
with col2:
    y = st.selectbox("Y-axis", numeric_cols, index=numeric_cols.index(top3[1]), key="by")
with col3:
    z = st.selectbox("Z-axis", numeric_cols, index=numeric_cols.index(top3[2]), key="bz")
...
st.download_button(
    ...
    file_name="behavioral_segments_filtered.csv",
    mime="text/csv"
)
